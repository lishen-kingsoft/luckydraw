...
export default function($scope, $meteor, $reactive, $timeout, $interval) {
  ...
  vm.helpers({
    ...
    dbLocalCandidates() {
      return Ppl.find({laId: '', loc: { $in: ['珠海', '广州', '香港'] }}, {}, {reactive: false});
    },
    dbGlobalCandidates() {
      return Ppl.find({gaId: ''}, {}, {reactive: false});
    }
  });
      ...
      if(oldStatus[0] && oldStatus[0].page != 'running' && newStatus[0] && newStatus[0].page == 'running') {
        ...
        if(lastFinished.scope == '小西山居') {
          _.each(vm.candidates, (candidate) => {
            Ppl.update({_id: candidate._id}, {$set:{laId: lastFinished._id}});
          });
        } else if (lastFinished.scope == '大西山居') {
          _.each(vm.candidates, (candidate) => {
            Ppl.update({_id: candidate._id}, {$set:{gaId: lastFinished._id}});
          });
        };
      }
    }, true);
